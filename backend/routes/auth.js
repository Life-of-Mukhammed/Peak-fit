...
router.post('/seed', async (req, res) => {
  try {
    const results = [];
    if (!await User.findOne({ login: 'kigo' })) {
      await new User({
        name: 'KiGo', surname: 'Platform', login: 'kigo', password: 'kigo123',
        role: 'platformAdmin',
        club: null,
        permissions: { kassa: false, mijozlar: false, ombor: false, xodimlar: false, tariflar: false, hisobotlar: false, sozlamalar: false },
      }).save();
      results.push('KiGo platforma admini yaratildi (login=kigo, parol=kigo123)');
    } else { results.push('KiGo platforma admini mavjud'); }
